...
public sealed class IntercomOpenApiHttpClient : IIntercomOpenApiHttpClient
{
    ...
    private readonly string _clientId = $"{nameof(IntercomOpenApiHttpClient)}:{Guid.NewGuid():N}";
    ...
    {
        return _httpClientCache.Get(_clientId, (config: _config, baseUrl: _config["Intercom:ClientBaseUrl"] ?? _prodBaseUrl), static state =>
        {
